    ...
    private class ClientHandler implements Runnable {
        ...
        private void handleAttack() throws IOException {
            ...
            if (random.nextInt(100) < hitChance) {
                ...
                sendNotify("You hit " + target.name + " for " + damage + ".");
                ...
                if (leveled) {
                    sendNotify("Your attack level increased!");
                }
                if (hpLeveled) {
                    player.refreshMaxHp();
                    sendNotify("Your hitpoints increased!");
                }
                if (target.hp == 0) {
                    handleMonsterDeath(target, player);
                }
            } else {
                sendNotify("You miss " + target.name + ".");
            }
        }

        private void handleMonsterDeath(Monster monster, ServerPlayer player) {
            monster.respawnAt = System.currentTimeMillis() + 8000;
            sendNotify(monster.name + " collapses.");
        ...
        private void handleInteract() throws IOException {
            ...
            if (node != null) {
                if (distance(player.x, player.y, tx, ty) > 1) {
                    sendNotify("You need to move closer.");
                    return;
                }
                ...
                sendNotify("You examine the " + node.name + ".");
                sendNotify("You take " + node.reward.name().toLowerCase() + " from the " + node.name + ".");
                if (leveled) {
                    sendNotify(node.skill.name().toLowerCase() + " level up!");
                }
                sendPlayerUpdate(player);
                return;
            }
            ...
            if (npc != null) {
                sendNotify("You talk to " + npc.name + ".");
                sendNotify(npc.dialogue[random.nextInt(npc.dialogue.length)]);
                if (npc.isShopkeeper) {
                    sendNotify("Shop: sells bread (6 coins), buys logs/ore/fish (5 coins). Use /buy bread or /sell log.");
                }
            }
        }
        ...
        private void handleShopBuy(String message) {
            if (!isNearShopkeeper()) {
                sendNotify("You need to be near the shopkeeper.");
                return;
            }
            String[] parts = message.split("\\s+");
            if (parts.length < 2) {
                sendNotify("Usage: /buy bread");
                return;
            }
            ...
            if ("bread".equals(item)) {
                if (player.inventory.remove(ItemType.COIN, 6)) {
                    player.inventory.add(ItemType.BREAD, 1);
                    sendNotify("You buy a loaf of bread.");
                    sendPlayerUpdate(player);
                } else {
                    sendNotify("Not enough coins.");
                }
            } else {
                sendNotify("The shop only sells bread right now.");
            }
        }
        ...
        private void handleShopSell(String message) {
            ...
            String[] parts = message.split("\\s+");
            if (parts.length < 2) {
                sendNotify("Usage: /sell log|ore|fish");
                return;
            }
            String item = parts[1].toLowerCase();
            ...
            if (type == ItemType.NONE) {
                sendNotify("I only buy logs, ore, and fish.");
                return;
            }
            if (player.inventory.remove(type, 1)) {
                player.inventory.add(ItemType.COIN, 5);
                sendNotify("Sold one " + item + ".");
                sendPlayerUpdate(player);
            } else {
                sendNotify("You don't have that item.");
            }
        }
        ...
        private void handleEquip(String message) {
            String[] parts = message.split("\\s+");
            if (parts.length < 2) {
                sendNotify("Usage: /equip sword|armor");
                return;
            }
            String item = parts[1].toLowerCase();
            if ("sword".equals(item)) {
                if (player.inventory.remove(ItemType.BRONZE_SWORD, 1)) {
                    ...
                    sendNotify("You equip a bronze sword.");
                    sendPlayerUpdate(player);
                } else {
                    sendNotify("You need a bronze sword.");
                }
                ...
                if (player.inventory.remove(ItemType.BRONZE_ARMOR, 1)) {
                    ...
                    sendNotify("You equip bronze armor.");
                    sendPlayerUpdate(player);
                } else {
                    sendNotify("You need bronze armor.");
                }
            } else {
                sendNotify("Unknown equipment.");
            }
        }
        ...
        private void handleEat(String message) {
            if (player.inventory.remove(ItemType.BREAD, 1)) {
                ...
                sendNotify("You eat the bread and feel better.");
                sendPlayerUpdate(player);
            } else {
                sendNotify("You have no bread.");
            }
        }
