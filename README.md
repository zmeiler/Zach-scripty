...
If PowerShell blocks venv activation with `PSSecurityException` / `running scripts is disabled`, run:
...
.\.venv\Scripts\python -m uvicorn apps.api.main:app --host 0.0.0.0 --port 8000 --reload
```

Or use CMD activation script instead:
...
## One-click option (recommended on Windows)

You can skip virtualenv setup and run a single-file standalone app:

```powershell
python standalone_ticker.py
```

This starts a local server at `http://127.0.0.1:8765`, opens your browser, and shows:
- live stock-style ticker updates
- live price/review table updates
- Pennsylvania dispensary directory

No external Python packages are required for this standalone mode.
